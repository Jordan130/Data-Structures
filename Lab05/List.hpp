...
struct ListNode
{
  ...
  ListNode (const T& v, ListNode* n, ListNode* p) 
  : data (v),
    next (n), 
    prev (p)
  {
...
public: /* should be private, but public for testing */
  ...
  transfer (iterator pos, iterator first, iterator last)
  {
    ...
    if (first != last) 
    {
      auto lastHooked = last.m_nodePtr->prev;
      Node::unhook_range (first.m_nodePtr, lastHooked);
      pos.m_nodePtr->hook_range (first.m_nodePtr, lastHooked);
    }
  }
...
public:
  ...
  {
    // TODO
    m_header.prev = m_header.prev->prev;
    delete m_header.prev->next;
  ...
  {
    // TODO -- EXTRA CREDIT [+5]
    // Hint: fixup all next/prev link
    iterator current = begin ();
    while(current != end ())
    {
      std::swap(current.m_nodePtr->prev, current.m_nodePtr->next);
      --current;
    }
